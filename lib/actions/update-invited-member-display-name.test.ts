...
describe("updateInvitedMemberDisplayName", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("returns an error when the display name is blank", async () => {
    const result = await updateInvitedMemberDisplayName("member-1", "   ");
  ...
  it("returns an error when not logged in", async () => {
    ...
    const result = await updateInvitedMemberDisplayName(
      "member-1",
      "山田太郎"
    );
  ...
  it("returns an error when RLS rejects the target (no rows updated)", async () => {
    // 他グループのメンバー、または対象が既に参加済み(user_idが埋まっている)場合に相当。
    mockedCreateClient.mockResolvedValue(
  ...
  it("succeeds for a valid target", async () => {
    mockedCreateClient.mockResolvedValue(
      fakeSupabase({
        data: { invited_display_name: "山田太郎" },
        error: null,
      })
    );
