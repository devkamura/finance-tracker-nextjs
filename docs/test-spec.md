...
| U-52 | `lib/receipts/amount-check.ts` | 正常系：同一税率の税別明細は小計に対して1回だけ端数処理する（インボイス制度準拠） | 税別98円×8%を3件 | 明細ごとの切り捨て合計(315円)ではなく小計切り捨て(317円)になる | なし |
| U-53 | `lib/receipts/amount-check.ts` | 正常系：税率が異なる税別明細はそれぞれの小計ごとに端数処理する | 税別98円×8%を2件＋税別1,000円×10%を1件 | 8%グループ211円＋10%グループ1,100円＝1,311円 | なし |
